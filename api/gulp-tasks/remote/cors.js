...
async function cors(config) {
  ...
  const lessonTypesPath = path.join(config.destFolder, config.destLessonTypes);
  ...
  const modulesByTypes = MODULE_TYPES.map((type) =>
    processListings(url, type, lessonTypes, config),
  );
...
export default cors;
export { REGULAR_SEMESTER, SPECIAL_SEMESTER, DATE_FORMAT };
